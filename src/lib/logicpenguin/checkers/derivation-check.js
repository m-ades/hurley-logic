...
export default class DerivationCheck {
    ...
    analyze(subderiv) {
        ...
        if (subderiv.showline &&
            ...
            if (!subderiv.showline.isMainConclusion && this.numberedShowLines) {
    ...
    checkConc() {
        ...
        // Recursively check all lines in all subderivations
        const checkLinesRecursive = (subderiv) => {
            if (!subderiv) return false;
            
            // Check lines in this subderivation first
            if (subderiv.lines && Array.isArray(subderiv.lines)) {
            ...
            // Recursively check parts (subderivations and lines)
            if (subderiv.parts && Array.isArray(subderiv.parts)) {
                for (let pt of subderiv.parts) {
                    if (!pt) continue;
                    
                    // If this part is a subderivation, check its lines recursively
                    if ("parts" in pt || "lines" in pt) {
                        if (checkLinesRecursive(pt)) {
                            return true;
                        }
                    }
                    
                    // Check if this part itself is a line with the conclusion
                    if (pt.s) {
                        try {
                            const ptNorm = Formula.from(pt.s).normal;
                            if (ptNorm == this.conc) {
                                return true;
                            }
                        } catch (e) {
        ...
        foundConc = checkLinesRecursive(this.deriv);
